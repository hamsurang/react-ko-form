...
| [validate](#validate)                                   | 폼 레벨 유효성 검사는 내장된 유효성 검사 메서드로만 제한됩니다.                                           |
| [progressive](/docs/useform/form)                       | `Form` 컴포넌트를 사용할 때 네이티브 폼 제출에 대한 점진적 향상을 활성화합니다.                                    |
...
이 예제는 React 애플리케이션에서 **새로운 `validate` API**를 `useForm`과 결합하여 **폼 레벨 유효성 검사**를 수행하는 방법을 보여줍니다.

`validate` 함수는 전체 폼 객체를 전달받으며, 이 함수를 통해 **구조화된 에러**를 반환하여 `formState.errors`와 통합할 수 있습니다.
...
- 사용자 정의 resolver를 만들 때:
  ...
  - `에러` 객체의 키는 필드의 `이름`과 일치해야 하지만, 깊은 에러의 경우 단일 키가 아닌 _계층적 구조여야만_ 합니다:
  `❌ { "participants.1.name": someErr }`는 제대로 설정되거나 해제되지 않습니다 - 대신 `✅ { participants: [null, { name: someErr } ] }`형태를 사용하세요. 이렇게 하면 `errors.participants[1].name`으로 접근할 수 있습니다 - 평탄한 키를 사용하여 에러를 준비한 다음, zod resolver의 다음 함수를 사용하여 변환할 수 있습니다:
