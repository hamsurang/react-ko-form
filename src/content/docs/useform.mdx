...
- 사용자 정의 resolver를 만들 때:
  ...
  `❌ { "participants.1.name": someErr }`는 제대로 설정되거나 제거되지 않습니다 - 대신 `✅ { participants: [null, { name: someErr } ] }`를 사용하세요. 이렇게 하면 `errors.participants[1].name`으로 접근할 수 있습니다 - 평탄한 키를 사용하여 에러를 준비한 다음, zod resolver의 다음 함수와 같은 것을 사용하여 변환할 수 있습니다:
