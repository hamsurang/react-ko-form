...
| [validate](#validate)                                   | 폼 수준 검증은 기본 제공되는 검증 메서드에 한정됩니다.                                                              |
...
이 예시는 React 애플리케이션에서 **폼 수준 검증**을 수행하기 위해 `useForm`과 **새로운 `validate` API**를 함께 사용하는 방법을 보여줍니다.

`validate` 함수는 전체 폼 객체를 수신하며 `formState.errors`와 통합되는 **구조화된 에러**를 반환할 수 있게 해줍니다.
