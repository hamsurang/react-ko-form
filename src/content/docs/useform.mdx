...
| [validate](#validate)                                   | 폼 수준 검증은 내장된 검증 메서드로 제한됩니다.                                                              |
...
이 예시는 **새로운 `validate` API**를 `useForm`과 조합하여 **폼 수준 검증**을 수행하는 방법을 보여줍니다.

`validate` 함수는 전체 폼 객체를 받고, `formState.errors`와 통합되는 **구조화된 에러**를 반환할 수 있습니다.
