...
| [validate](#validate)                                   | 폼 수준 검증은 기본 제공 검증 메서드로 한정됩니다.                                                              |
...
| [progressive](/docs/useform/form)                       | `Form` 컴포넌트를 사용할 때 네이티브 폼 제출을 위한 점진적 향상(progressive enhancement)을 활성화합니다.                                    |
...
`validate` 함수는 전체 폼 객체를 수신하며, `formState.errors`와 통합된 **구조화된 에러**를 반환할 수 있게 해줍니다.
