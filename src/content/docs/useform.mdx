...
| [progressive](/docs/useform/form)                       | `Form` 컴포넌트를 사용할 때 네이티브 폼 전송을 위한 점진적 향상을 활성화합니다.                                    |
...
이 예제는 **새로운 `validate` API**를 사용하여 `useForm`과 결합해 **폼 수준 검증**을 수행하는 방법을 보여줍니다.

`validate` 함수는 전체 폼 객체를 수신하며, `formState.errors`와 통합된 **구조화된 에러**를 반환할 수 있게 합니다.
