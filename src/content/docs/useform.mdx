...
| [validate](#validate)                                   | 폼 수준 검증은 내장된 검증 방법으로 제한됩니다.                                                              |
...
| [progressive](/docs/useform/form)                       | `Form` 컴포넌트를 사용할 때 기본 폼 제출을 위한 점진적 개선을 활성화합니다.                                    |
...
이 예제는 React 애플리케이션에서 **폼 수준 검증**을 수행하기 위해 `useForm`과 결합된 **새로운 `validate` API**를 사용하는 방법을 보여줍니다.

`validate` 함수는 전체 폼 객체를 수신하고 `formState.errors`와 통합되는 **구조화된 에러**를 반환할 수 있게 해줍니다.
