...
React Hook Form은 비제어 컴포넌트와 기본 입력 요소를 지향하지만, [React-Select](https://github.com/JedWatson/react-select), [AntD](https://github.com/ant-design/ant-design), [MUI](https://mui.com/)와 같은 외부 제어 컴포넌트를 React Hook Form과 같이 사용하지 않도록 하는 것은 어렵습니다. 이 래퍼(wrapper) 컴포넌트는 이러한 작업을 더욱 쉽게 만들어줍니다.
...
| `defaultValue`     | <TypeText>unknown</TypeText>                                       |          | **중요:** `useForm`의 `defaultValue` 또는 `defaultValues`에서 `undefined`를 사용할 수 없습니다. <ul><li>필드 레벨에서 `defaultValue`나 `useForm`의 `defaultValues`를 설정해야 합니다. `undefined`는 유효한 값이 아닙니다.</li><li>폼에서 기본값으로 `reset`을 호출할 경우,`useForm`에 `defaultValues`를 제공해야 합니다.</li><li>`undefined`로 `onChange`를 호출하는 것은 유효하지 않습니다. 대신 `null`이나 빈 문자열을 기본값이나 초기화된 값으로 사용해야 합니다. </li></ul> |
...
| `disabled`         | <TypeText>boolean = false`</TypeText>                              |          | `disabled` 속성은 `field` 속성에서 반환됩니다. 제어 입력 필드는 비활성화되며, 그 값은 제출 데이터에서 제외됩니다.                                                                                                                                                                                                                                                                                                                                                              |
...
| -------------------------------------- | -------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `field`                                | `onChange`           | <TypeText>`(value: any) => void`</TypeText> | 입력 값을 라이브러리에 전송하는 함수.<br/><br/> _이 함수는 입력 필드의 `onChange` prop에 할당해야 하며, 값은 **`undefined`가 될 수 없습니다**._ <br/>이 prop은 [formState](/docs/useform/formstate)를 업데이트하므로, [setValue](/docs/useform/setvalue)나 필드 업데이트와 관련된 다른 API를 수동으로 호출하는 것은 피해야 합니다.                                                                                                                                                                                                               |
| `field`                                | `onBlur`             | <TypeText>`() => void`</TypeText>           | 입력의 onBlur 이벤트를 라이브러리로 전송하는 함수. 이 함수는 입력 필드의 `onBlur` prop에 할당되어야 합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `formState`                            | `submitCount`        | <TypeText>number</TypeText>                 | 폼이 제출된 횟수.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `formState`                            | `isValid`            | <TypeText>boolean</TypeText>                | 폼에 에러가 없으면 `true`로 설정합니다.<br/><br/>`setError` `isValid` formState에 영향을 미치지 않으며, `isValid`는 항상 전체 폼 유효성 검사 결과를 통해 도출됩니다.                                                                                                                                                                                                                                                                                                                                                                           |
| `formState`                            | `isValidating`       | <TypeText>boolean</TypeText>                | 유효성 검사를 하고 있는 동안 `true`로 설정합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `formState`                            | `errors`             | <TypeText>object</TypeText>                 | 필드 에러를 담고 있는 객체. 에러 메세지를 쉽게 가져오기 위한 [ErrorMessage](/docs/useformstate/errormessage) 컴포넌트도 있습니다.                                                                                                                                                                                                                                                                                                                                                                                                              |
    ...
    <form onSubmit={handleSubmit((data) => console.log(data))}>
      <Controller
        ...
        render={({ field: { onChange, onBlur, value, ref } }) => (
          <ReactDatePicker
            onChange={onChange} // hook form에 값 전송
...
- MUI, AntD, Chakra UI와 같은 외부 제어 컴포넌트를 사용할 때 각 prop의 역할을 이해하는 것이 중요합니다. Controller는 값을 알리고 설정함으로써 입력 필드의 "스파이" 역할을 합니다.
  - **onChange**: hook form으로 다시 데이터를 전송.
  - **onBlur**: 입력이 상호작용되었음을 보고(focus 및 blur).
  - **value**: 입력 필드의 초기 값과 업데이트된 값을 설정.
  - **ref**: 에러가 있는 입력 필드에 포커스를 허용.
...
- 입력을 다시 등록(`register`)하지 마세요. 이 컴포넌트는 등록(registration) 과정을 처리하기 위해 설계되었습니다.
...
- `onChange` 동안 값을 변환하여 hook form에 전송될 값을 커스터마이즈하세요.
  ```javascript
  <Controller
    name="test"
    render={({ field }) => {
