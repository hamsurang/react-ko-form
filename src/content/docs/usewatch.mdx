...
`watch` API와 유사하게 동작하지만, 이 훅은 리렌더링을 커스텀 훅 수준에서 격리하여 애플리케이션의 성능을 향상시킬 수 있습니다.
...
- `useWatch`와 `watch`의 유일한 차이점은 루트([`useForm`](/docs/useform)) 레벨 또는 커스텀 훅 수준에서의 업데이트에 있습니다.
- `useWatch`의 실행 순서는 중요합니다. 즉, 구독이 설정되기 전에 폼 값을 업데이트 하면, 업데이트된 값이 무시됩니다.

  ```javascript copy
  setValue("test", "data")
  useWatch({ name: "test" }) // ❌ 값이 업데이트 된 후 구독이 발생하고, 업데이트가 무시됨
  useWatch({ name: "example" }) // ✅ 입력 값 업데이트가 반영되고, 리렌더링이 트리거됨
  setValue("example", "data")
  ```

  위 문제는 간단한 커스텀 훅을 사용하여 해결할 수 있습니다:
  ...
  const useFormValues = () => {
    ...
    return {
      ...useWatch(), // 폼 값 업데이트를 구독하세요.

      ...getValues(), // 항상 최신 폼 값과 병합하세요.
    }
  }
  ```
...
function FirstNameWatched({ control }: { control: Control<FormInputs> }) {
  const firstName = useWatch({
    control,
    name: "firstName", // 이름을 제공하지 않으면 전체 폼을 감시하게 되고,['firstName', 'lastName']을 사용하면 둘다 감시할 수 있습니다.
    defaultValue: "default", // 렌더링 전 기본값
  })

  return <p>Watch: {firstName}</p> // firstName이 변경될 때만 커스텀 훅 레벨에서 리렌더링하세요.
}
...
export const Calc = ({ control, setValue }) => {
  ...
  // 필드 배열을 사용하여 결과를 계산하기 위해 독립적인 리렌더링
  console.log(results)
