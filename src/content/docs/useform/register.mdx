...
이름은 숫자로 시작하거나 독립된 키로 숫자를 사용할 수 없으며, 특수 문자를 피해야 합니다.  
TypeScript의 일관성을 위해, 점(.) 문법만 지원되며 대괄호([]) 문법은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li>각 렌더에서 인풋의 <code>name</code>을 변경하면 필드가 새로 등록됩니다.  
