...
이름은 숫자로 시작하거나 숫자를 독립적인 키로 사용할 수 없으며, 특수 문자는 피해야 합니다.
타입스크립트의 일관성을 위해 점(. ) 구문만 지원되며, 대괄호([]) 구문은 배열 폼 값에 대해 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 그 폼 값으로 <code>undefined</code>를 반환합니다.
        값을 유지하면서 사용자 편집을 방지해야 하는 경우, 
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참조하세요.</li>
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
  <li>각 렌더에서 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.
        일관된 동작을 보장하려면 렌더 간에 인풋 이름을 안정적으로 유지하세요.</li>
  <li>
  <li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.
        필요할 경우 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하여 명시적으로 제거하세요.</li>
