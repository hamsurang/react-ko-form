...
이름은 숫자로 시작할 수 없으며, 숫자를 독립된 키로 사용해서는 안 되고, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점(.) 구문만 지원됩니다—브래킷(<code>[]</code>) 구문은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값은 유지하면서 사용자 편집을 방지하려면 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기에 대한 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
