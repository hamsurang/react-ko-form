...
이름은 숫자로 시작해서는 안되며, 숫자를 독립 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
타입스크립트의 일관성을 위해 점(dot) 문법만 지원됩니다—중괄호(bracket) 문법(<code>[]</code>)은 배열 폼 값에 적용되지 않습니다.  
...
일관된 동작을 보장하려면 인풋 이름을 렌더 간 안정적으로 유지하세요.</li>  
  <li>
