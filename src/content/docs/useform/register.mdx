...
이름은 숫자로 시작할 수 없으며, 숫자를 독립적으로 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
타입스크립트 일관성을 위해 점(.) 문법만 지원됩니다—대괄호([]) 문법은 배열 폼 값에 작동하지 않습니다.  
...
사용자 수정을 방지하면서 값을 유지하려면 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 다음은 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
