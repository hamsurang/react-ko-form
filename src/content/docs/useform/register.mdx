...
타입스크립트 일관성을 위해 점 구문만 지원되며—대괄호 구문(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정을 방지하려면 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 아래는 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
