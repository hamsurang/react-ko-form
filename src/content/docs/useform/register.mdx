...
<ul>
  <li>
<strong>Name</strong>는 <strong>필수</strong>이며 <strong>고유해야</strong> 합니다  
(네이티브 라디오 및 체크박스 입력을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작해서는 안 되며, 숫자를 독립적인 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 폼 값으로 <code>undefined</code>를 반환합니다.  
        값을 유지하면서 사용자 수정을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
