...
이름은 숫자로 시작하거나 standalone 키로 숫자를 사용해서는 안 되며, 특수 문자를 피해야 합니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정을 방지해야 하는 경우 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [여기](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx) 예시가 있습니다.</li>  
