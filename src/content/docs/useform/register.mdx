...
<ul>
  <li>
<strong>Name</strong>은 <strong>필수</strong>이고 <strong>고유해야</strong> 합니다  
(내장 라디오 및 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작하거나 숫자를 독립적인 키로 사용할 수 없으며 특별한 문자는 피해야 합니다.  
...
<li> 비활성화된 인풋은 그들의 폼 값으로 <code>undefined</code>를 반환합니다.  
        값은 유지하면서 사용자 편집을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 예시가 있습니다: [example](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx).</li>  
<li>각 렌더링마다 인풋의 <code>name</code>을 변경하면 새로운 필드로 재등록됩니다.  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
        일관된 동작을 보장하기 위해서는 렌더링 사이에 인풋 이름을 안정적으로 유지하세요.</li>  
