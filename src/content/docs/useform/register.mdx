...
(본래 라디오 및 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작할 수 없으며 단독 키로 숫자를 사용해서는 안 되며, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점(dot) 구문만 지원되며, 대괄호(bracket) 구문(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code> 또는 전체 <code>fieldset</code>을 비활성화하십시오. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참조하세요.</li>  
