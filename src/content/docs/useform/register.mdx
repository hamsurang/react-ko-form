...
이름은 숫자로 시작해서는 안 되며, 숫자를 독립적인 키로 사용할 수 없고, 특수 문자는 피해야 합니다.  
타입스크립트 일관성을 위해 점(dot) 구문만 지원됩니다—대괄호(<code>[]</code>) 구문은 배열 폼 값에 대해 작동하지 않습니다.  
