...
이름은 숫자로 시작하거나 독립 키로 숫자를 사용해서는 안 되며, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점 구문만 지원되며—대괄호 구문(<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        사용자의 편집을 방지하면서 값을 유지해야 하는 경우,  
        <code>readOnly</code> 또는 전체 <code>fieldset</code>을 비활성화하세요. 여기에 대한 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
