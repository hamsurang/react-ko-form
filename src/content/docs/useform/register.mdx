...
이름은 숫자로 시작해서는 안 되며, 숫자를 독립된 키로 사용할 수 없으며, 특수 문자는 피해야 합니다.
타입스크립트 일관성을 위해, 점(dot) 문법만 지원됩니다—브래킷 문법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.
        값을 유지하면서 사용자의 수정을 방지해야 하는 경우,
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
  <li>매번 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 리등록됩니다.
