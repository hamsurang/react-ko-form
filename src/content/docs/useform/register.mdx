...
이름은 숫자로 시작해선 안 되며, 숫자를 독립적인 키로 사용할 수 없고, 특수 문자는 피해야 합니다.  
타입스크립트 일관성을 위해 점(dot) 구문만 지원되며, 괄호(bracket) 구문 (<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>  
