...
이름은 숫자로 시작할 수 없으며, 독립적인 키로 숫자를 사용해서는 안 되며, 특수 문자를 피해야 합니다.  
타입스크립트 일관성을 위해, 점 표기법만 지원됩니다—배열 폼 값에 대한 대괄호 표기법(<code>[]</code>)은 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        사용자가 수정하지 못하도록 하면서 값을 유지하려면,  
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
