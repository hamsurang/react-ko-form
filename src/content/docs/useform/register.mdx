...
(기본 라디오 및 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작해서는 안 되며 독립적으로 숫자를 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
타입스크립트 일관성을 위해 점(.) 구문만 지원됩니다—대괄호 구문 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값은 유지하면서 사용자 편집을 방지해야 하는 경우,  
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
...
<li>각 렌더링에서 인풋의 <code>name</code>을 변경하면 새로운 필드로 재등록됩니다.  
  ...
  <li>
<li>인풋 값 및 참조는 언마운트 시 자동으로 제거되지 않습니다.  
