...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 편집을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
  ...
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
        일관된 동작을 보장하려면 렌더링 간 입력 이름을 안정적으로 유지하세요.</li>  
  <li>입력 값 및 참조는 언마운트 시 자동으로 제거되지 않습니다.  
  <li>
