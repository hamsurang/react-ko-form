...
이름은 숫자로 시작하거나 숫자를 단독 키로 사용할 수 없으며, 특수 문자를 피해야 합니다.
타입스크립트 일관성을 위해 점(dot) 문법만 지원됩니다—대괄호([]) 문법은 배열 폼 값에 대해 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 해당 폼 값으로 <code>undefined</code>를 반환합니다.
        값을 유지하면서 사용자 수정을 방지하려면 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>
