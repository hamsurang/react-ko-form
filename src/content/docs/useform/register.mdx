...
(네이티브 라디오 및 체크박스 입력 제외).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작할 수 없으며 독립적인 키로 숫자를 사용해서는 안 되고, 특수 문자는 피해야 합니다.  
타입스크립트의 일관성을 위해 점 표기법만 지원되며—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
