...
타입스크립트 일관성을 위해, 점 구문만 지원됩니다—배열 폼 값에 대한 괄호 구문 (<code>[]</code>)은 작동하지 않습니다.  
...
사용자가 편집하지 못하도록 방지하면서 값을 유지해야 하는 경우,  
<code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기에 대한 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
...
일관된 동작을 보장하려면 렌더링 간에 인풋 이름을 안정적으로 유지하세요.</li>  
  <li>
<li>인풋 값과 레퍼런스는 언마운트 시 자동으로 제거되지 않습니다.  
