...
TypeScript의 일관성을 위해, 점 표기법만 지원되며—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
사용자 편집을 방지하면서 값을 유지하려면 <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. 다음은 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
