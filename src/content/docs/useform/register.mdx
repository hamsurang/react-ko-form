...
<ul>
  <li>
<strong>이름</strong>은 <strong>필수</strong>이며 <strong>고유해야</strong> 합니다  
(네이티브 라디오 및 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작해서는 안 되며, 독립적인 키로 숫자를 사용해서는 안 되고, 특수 문자는 피해야 합니다.  
TypeScript의 일관성 유지를 위해 점(dot) 구문만 지원됩니다—대괄호 구문(<code>[]</code>)은 평면 필드 배열 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 해당 폼 값으로 <code>undefined</code>를 반환합니다.  
        값을 유지하면서 사용자의 수정을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참조하세요.</li>  
  <li>각 렌더링마다 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
        일관된 동작을 보장하려면 렌더링 간에 인풋 이름을 안정적으로 유지하십시오.</li>  
  <li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
  <li>
        필요할 때 명시적으로 제거하려면 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하십시오.</li>  
