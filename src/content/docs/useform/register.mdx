...
이름은 숫자로 시작할 수 없으며, 숫자를 단독 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
타입스크립트 일관성을 위해, 점 문법만 지원되며—대괄호 문법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 편집을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참조하세요.</li>  
...
<li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
  <li>
        필요할 때는 명시적으로 제거하기 위해 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하세요.</li>  
