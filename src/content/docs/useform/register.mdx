...
이름은 숫자로 시작할 수 없으며, 숫자를 독립된 키로 사용할 수 없고, 특수 문자는 피해야 합니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정 방지를 원하면, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>  
...
<li>입력 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
  <li>
        필요 시 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용해 명시적으로 제거하세요.</li>  
