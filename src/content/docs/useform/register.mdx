...
TypeScript 일관성을 위해서 점(dot) 문법만 지원됩니다—대괄호 문법(<code>[]</code>)은 배열 폼 값에 대해서 작동하지 않습니다.  
...
<li>인풋의 <code>name</code>을 매 렌더링마다 변경하면 새로운 필드로 리등록됩니다.  
