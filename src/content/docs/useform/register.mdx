...
(네이티브 라디오와 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작할 수 없으며 독립적으로 사용되는 숫자를 포함해서는 안 되며, 특수 문자를 피해야 합니다.  
타입스크립트 일관성을 위해 점(dot) 구문만 지원되며—대괄호(bracket) 구문(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 폼 값으로 <code>undefined</code>를 반환합니다.  
        값을 유지하면서 사용자 수정을 방지해야 하는 경우,  
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
<li>각 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.  
        일관된 동작을 보장하려면 인풋 이름을 렌더링 간에 안정적으로 유지하세요.</li>  
  <li>
<li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
        필요 시 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하여 명시적으로 제거하세요.</li>  
