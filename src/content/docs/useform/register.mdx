...
이름은 숫자로 시작해서는 안 되며, 숫자를 독립 키로 사용해서는 안 되며, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해서는 점 문법만 지원되며—대괄호 문법 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화합니다. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>  
  ...
  <li>각 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.  
        일관된 동작을 보장하려면 렌더링 간 인풋 이름을 안정적으로 유지하세요.</li>  
  <li>
  <li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
        필요할 때는 명시적으로 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하여 제거하세요.</li>
