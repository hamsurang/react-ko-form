...
TypeScript 일관성을 위해, 점(.) 문법만 지원됩니다—대괄호([]) 문법은 배열 폼 값에 적용되지 않습니다.  
...
<li> 비활성화된 입력은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값은 유지하면서 사용자 수정을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 예제는 [여기](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참고하세요.</li>  
<li>각 렌더에서 인풋의 <code>name</code>을 변경하면 새 필드로 리등록됩니다.  
