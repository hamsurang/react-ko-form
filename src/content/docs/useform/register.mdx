...
<ul>
  <li>
<strong>이름</strong>은 <strong>필수</strong>이며 <strong>고유해야 합니다</strong> 
...
타입스크립트 일관성을 위해 점 표기법만 지원되며, 괄호 표기법 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성 입력은 자신의 폼 값으로 <code>undefined</code>를 반환합니다.
        값은 보존하면서 사용자 편집을 방지해야 하는 경우, 
  ...
  <li>매 렌더링마다 입력의 <code>name</code>을 변경하면 새로운 필드로 리등록됩니다.
        일관된 동작을 보장하려면, 렌더링 간에 입력 이름을 안정적으로 유지하세요.</li>
  <li>
