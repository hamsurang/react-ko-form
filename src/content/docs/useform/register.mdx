...
이름은 숫자로 시작할 수 없으며, 숫자를 독립적으로 키로 사용할 수 없고, 특수 문자는 피해야 합니다.
TypeScript 일관성을 위해, 점 표기법만 지원됩니다—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 대해서 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.
        값은 유지하면서 사용자 수정을 방지해야 하는 경우,
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>를 비활성화하세요. 다음은 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
  <li>인풋의 <code>name</code>이 매 렌더링마다 변경되면 새 필드로 리등록됩니다.
        일관된 동작을 보장하기 위해, 렌더링 간 인풋 이름을 안정적으로 유지하세요.</li>
  <li>
