...
TypeScript 일관성을 위해서는 점 표기법만 지원됩니다—브래킷 표기법(<code>[]</code>)은 배열 폼 값에 사용할 수 없습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li>비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code> 또는 전체 <code>fieldset</code>을 비활성화하세요. 여기  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
        에는 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
