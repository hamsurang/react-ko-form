...
이름은 숫자로 시작하거나 독립적인 키로 숫자를 사용해서는 안 되며, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점(.) 구문만 지원됩니다—대괄호([]) 구문은 배열 폼 값에 대해 작동하지 않습니다.  
...
사용자 수정은 방지하면서 값을 유지하려면, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요.  
여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
