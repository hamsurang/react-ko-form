...
타입스크립트 일관성을 위해 점(dot) 문법만 지원되며, 대괄호(bracket) 문법(<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정 방지를 해야 한다면, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
