...
이름은 숫자로 시작할 수 없으며, 숫자를 독립적인 키로 사용할 수 없고, 특수 문자를 피해야 합니다.  
TypeScript 일관성을 위해 점 표기법만 지원되며—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li>각 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 리등록됩니다.  
