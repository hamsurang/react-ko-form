...
- **`name`**은 **필수**이며 **고유해야** 합니다(**네이티브 라디오 및 체크박스 입력은 예외**).
- 입력 필드의 이름은 점(dot) 표기법 또는 대괄호(bracket) 문법을 사용할 수 있으며, 이를 통해 중첩된 폼 필드를 손쉽게 생성할 수 있습니다.

```jsx
register("test.0.firstName") // ✅
register("test[0].firstName") // ❌

- 이름은 숫자로 시작할 수 없으며, 숫자가 단독 키 이름으로 쓰이지 않아야 합니다. 특수 문자의 사용은 피하세요.
- TypeScript 일관성을 위해 점(dot) 표기법만 지원됩니다. 따라서 배열 형식의 폼 값에는 대괄호 표기법(`[ ]`)을 사용할 수 없습니다.
- 비활성화된 입력 요소는 `undefined` 값을 반환합니다. 입력 값 변경을 막고 싶지만 값을 유지하려면 `readOnly`를 사용하거나 전체 `fieldset`을 비활성화하세요. 자세한 내용은 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참고하세요.
- 필드 배열을 생성하려면 입력 필드 이름 뒤에 점과 숫자를 추가하세요. 예: `test.0.data`
- 입력의 `<code>name</code>`을 렌더링할 때마다 변경하면 해당 입력이 새로운 필드로 다시 등록됩니다. 일관된 동작을 보장하려면 입력 이름을 렌더 간에 고정하세요.
- 인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다. 필요할 경우, [<code>unregister</code>](https://react-hook-form.com/docs/useform/unregister)을 호출하여 명시적으로 제거하세요.
### Props
...
- `name`은 **필수**이고 **고유**해야 합니다(네이티브 라디오 및 체크박스는 제외).
- `name`은 점(dot) 표기법과 대괄호(bracket) 표기법을 모두 지원하여 네스티드 폼 필드를 쉽게 생성할 수 있습니다. 하지만, **TypeScript 사용의 일관성을 위해 점 표기법만 지원**하며, 대괄호([]) 표기법은 배열 형태의 폼 값에서 작동하지 않습니다. 
...
- `name`은 숫자로 시작하거나 숫자를 단독 키 이름으로 사용할 수 없으며, 특수 문자의 사용도 피해야 합니다.
- 비활성화된(disabled) 인풋은 `undefined` 값으로 처리됩니다. 인풋의 값을 유지하면서도 사용자가 편집하지 못하도록 하려면, `readOnly`를 사용하거나 `fieldset` 전체를 비활성화하십시오. [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 참고하세요.
- 필드 배열을 생성하려면, 입력 이름 뒤에 점과 숫자를 사용하세요. 예: `test.0.data`
- 매 렌더링 시 이름을 변경하면 새로운 인풋이 등록되므로, 등록(register)된 각 인풋에 대해 이름을 고정하는 것이 좋습니다.
- 인풋 값과 레퍼런스는 언마운트 시 자동으로 제거되지 않습니다. 필요할 경우, 명시적으로 제거하려면 [`unregister`](/docs/useform/unregister)를 호출하세요.
### Return
...
| <TypeText>register("test.0.firstName")</TypeText>   | <TypeText>`{ test: [ { firstName: value } ] }`</TypeText> |
| <TypeText>register("test[0].firstName")</TypeText>  | <TypeText>`{}` (지원 안 됨)                               |
