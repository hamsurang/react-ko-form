...
이름은 숫자로 시작하거나 숫자를 독립적인 키로 사용할 수 없으며, 특수 문자를 피해야 합니다.
TypeScript 일관성을 위해 점 표기법만 지원됩니다—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.
        값은 유지하면서 사용자의 수정을 방지해야 하는 경우,
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기에 대한 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
  <li>각 렌더링마다 인풋의 <code>name</code>을 변경하면 새 필드로 다시 등록됩니다.
        일관된 동작을 보장하려면 렌더링 간 입력 이름을 안정적으로 유지해야 합니다.</li>
  <li>
  <li>입력 값과 참조는 언마운트 시 자동으로 삭제되지 않습니다.
