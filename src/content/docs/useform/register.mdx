...
사용자의 수정을 방지하면서 값을 유지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
<li>인풋의 <code>name</code>을 매 렌더링 시 변경하면 새로운 필드로 리등록됩니다.  
