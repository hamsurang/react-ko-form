...
이름은 숫자로 시작하지 않아야 하며, 독립적으로 사용되는 숫자를 사용할 수 없고, 특수 문자는 피해야 합니다.  
TypeScript의 일관성을 위해, 오직 점 구문만 지원됩니다—대괄호 구문 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
값을 유지하면서 사용자 수정을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [여기](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx) 예제가 있습니다.</li>  
