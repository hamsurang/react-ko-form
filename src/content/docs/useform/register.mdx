...
TypeScript 일관성을 위해 도트 문법만 지원됩니다—대괄호 문법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. 여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
<li>각 렌더링에서 인풋의 <code>name</code>을 변경하면 새 필드로 다시 등록됩니다.  
        일관된 동작을 보장하려면 렌더링 간에 인풋 이름을 안정적으로 유지하십시오.</li>  
  <li>
<li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
        필요할 경우 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하여 명시적으로 제거하십시오.</li>  
