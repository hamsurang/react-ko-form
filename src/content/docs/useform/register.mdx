...
이름은 숫자로 시작하거나 숫자를 독립적으로 키로 사용하는 것은 안 되며, 특수 문자를 피해야 합니다.  
타입스크립트의 일관성을 위해, 점 표기법만 지원됩니다 — 대괄호 표기법 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li>비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 유지하면서 사용자 수정 방지를 원한다면, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>  
  <li>매 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 재등록됩니다.  
