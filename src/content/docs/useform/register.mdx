...
이름은 숫자로 시작해서는 안 되며, 숫자를 독립적인 키로 사용하지 말아야 하고, 특수 문자를 피해야 합니다.
타입스크립트 일관성을 위해 점 표기법만 지원됩니다—브래킷 표기법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li>각 렌더마다 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.
         일관된 동작을 보장하기 위해 렌더 사이에 인풋 이름을 안정적으로 유지하세요.</li>
  <li>
