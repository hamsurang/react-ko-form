...
이름은 숫자로 시작해서는 안 되며, 숫자를 단독 키로 사용할 수 없고, 특수 문자는 피해야 합니다.  
TypeScript의 일관성을 위해 점(dot) 문법만 지원됩니다—대괄호(bracket) 문법(<code>[]</code>)은 배열 폼 값에는 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 form 값으로 <code>undefined</code>를 반환합니다.  
사용자의 편집을 방지하면서 값을 유지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)를 확인하세요.</li>  
<li>각 렌더링 시 인풋의 <code>name</code>을 변경하면 새로운 필드로 재등록됩니다.  
