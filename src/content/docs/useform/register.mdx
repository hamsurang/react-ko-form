...
이름은 숫자로 시작해서는 안 되며, 독립적인 키로 숫자를 사용하지 않아야 하고, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점 구문만 지원됩니다—대괄호 구문 (<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값은 유지하면서 사용자 편집을 방지하려면, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 여기 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
  ...
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
        일관된 동작을 보장하려면 렌더링 사이에 인풋 이름을 안정적으로 유지하세요.</li>  
