...
TypeScript의 일관성을 위해 점(.) 구문만 지원됩니다—브래킷 구문 (<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        값을 보존하면서 사용자 수정 방지를 원한다면  
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 다음은 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
...
<li>인풋 값과 참조는 언마운트 시 자동으로 제거되지 않습니다.  
        필요 시 명시적으로 제거하려면 <a href="/docs/useform/unregister"><code>unregister</code></a>를 사용하세요.</li>  
