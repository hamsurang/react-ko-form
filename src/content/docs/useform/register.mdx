...
<ul>
  <li>
<strong>이름</strong>은 <strong>필수</strong>이며 <strong>고유</strong>해야 합니다  
(네이티브 라디오 및 체크박스 인풋을 제외하고).  
  <li>
    이름은 은 숫자로 시작하거나 키 이름으로 숫자를 사용할 수 없습니다. 특수 문자의 사용도 피해야 합니다.
이름은 숫자로 시작하거나 숫자를 독립적으로 키로 사용할 수 없으며, 특수 문자는 피해야 합니다.  
TypeScript의 일관성을 위해 점 표기법만 지원됩니다—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 해당 폼 값으로 <code>undefined</code>를 반환합니다.  
        값은 유지하면서 사용자 편집을 방지해야 하는 경우, <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 다음은 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
