...
이름은 숫자로 시작하거나 독립적인 키로 숫자를 사용할 수 없으며, 특수 문자는 피해야 합니다.  
...
register("test[0].firstName") // ❌  
  ...
  <li> 비활성화된 인풋은 해당 폼 값으로 <code>undefined</code>를 반환합니다.  
        사용자 수정을 방지하면서 값을 유지해야 하는 경우,  
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>를 비활성화하세요. 여기에 대한 [예시](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
