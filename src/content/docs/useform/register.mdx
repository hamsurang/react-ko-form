...
이름은 숫자로 시작할 수 없으며, 독립적인 키로 숫자를 사용할 수 없고, 특수 문자는 피해야 합니다.
TypeScript 일관성을 위해 점 구문만 지원됩니다—대괄호 구문(<code>[]</code>)은 배열 폼 값에 적용되지 않습니다.
...
register("test[0].firstName") // ❌
  ...
  <li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.
        값을 유지하면서 사용자 수정을 방지하려면
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. 여기에 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>
  ...
  <li>각 렌더링에서 인풋의 <code>name</code>을 변경하면 새 필드로 다시 등록됩니다.
        일관된 동작을 보장하기 위해 렌더링 전반에 걸쳐 인풋 이름을 안정적으로 유지하십시오.</li>
  <li>
