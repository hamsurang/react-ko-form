...
이름은 숫자로 시작하지 않거나 숫자를 독립적으로 키로 사용할 수 없으며, 특수 문자는 피해야 합니다.  
TypeScript 일관성을 위해 점 표기법만 지원됩니다—대괄호 표기법(<code>[]</code>)은 배열 폼 값에 대해 작동하지 않습니다.  
...
<li> 비활성화된 인풋은 <code>undefined</code>를 폼 값으로 반환합니다.  
        ...
        <code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하십시오. 여기 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)가 있습니다.</li>  
...
<li> 각 렌더링마다 인풋의 <code>name</code>을 변경하면 새로운 필드로 다시 등록됩니다.  
        일관된 동작을 보장하려면 렌더링 간 인풋 이름을 안정적으로 유지하십시오.</li>  
  <li>
