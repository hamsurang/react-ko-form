...
이름은 숫자로 시작해서는 안 되며, 독립적인 키로 숫자를 사용할 수 없고, 특수 문자를 피해야 합니다.  
TypeScript 일관성을 위해 점 표기법만 지원되며—대괄호 표기법 (<code>[]</code>)은 배열 폼 값에 사용될 수 없습니다.  
...
사용자의 수정을 방지하면서 값을 유지하려면,  
<code>readOnly</code>를 사용하거나 전체 <code>fieldset</code>을 비활성화하세요. 아래는 [예제](https://codesandbox.io/s/react-hook-form-disabled-inputs-oihxx)입니다.</li>  
  <li>언마운트에 따라 입력 값과 참조는 더 이상 제거되지 않습니다. 해당 값과 참조를 제거하려면 unregister를 호출할 수 있습니다.</li>
<li> 각 렌더링에서 인풋의 <code>name</code>을 변경하면 새 필드로 재등록됩니다.  
