...
- 입력 필드와 연관되지 않은 에러는 `clearErrors`를 통해 지워질 때까지 지속됩니다. 이 동작은 필드 수준에서의 내장 유효성 검사에만 해당됩니다.
  ...
  ```
- `root`를 키로 서버 또는 전역 에러를 설정할 수 있습니다. 이러한 유형의 에러는 각 제출마다 유지되지 않습니다.
  ```javascript
...
- 이 메서드는 `isValid` 폼 상태를 `false`로 강제 설정한다. 그러나 `isValid`는 항상 입력 등록 규칙 또는 스키마 결과의 유효성 검사 결과에서 파생된다는 점에 유의해야 한다.
- 타입 검사를 방해하지 않기 위해 `type`과 `types`라는 키워드는 피해야 한다.
