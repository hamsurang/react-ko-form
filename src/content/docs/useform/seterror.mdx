...
- 이 메서드는 <code>formState.isValid</code>를 <code>false</code>로 강제 설정합니다. 그러나 <code>isValid</code>는 항상 입력 등록 규칙 또는 스키마 결과의 유효성 검사 결과에서 파생된다는 점에 유의해야 합니다.
- 타입 검사를 방해하지 않기 위해 특정 키워드(예: `type`과 `types`)를 피해야 합니다.
