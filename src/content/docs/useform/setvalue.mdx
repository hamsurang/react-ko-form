...
|                                          | `shouldDirty`<br/><TypeText>boolean</TypeText>    | <ul><li>`defaultValues`과 비교해서 해당 입력이 수정되었는지 판단할지 여부를 결정. (구독 대상: <TypeText>dirtyFields</TypeText>).</li><li>`defaultValues`과 비교해서 전체 폼이 수정되었는지 판단할지 여부를 결정. (구독 대상: <TypeText>isDirty</TypeText>).</li><li>이 옵션은 전체 폼 필드의 레벨이 아닌 특정 필드 레벨에서만 `dirtyFields`를 업데이트합니다. </li></ul> |
