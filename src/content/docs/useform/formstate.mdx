...
| `isReady`            | <TypeText>boolean</TypeText> | `formState` 구독 설정이 준비되면 `true`로 설정됩니다. <ul><li><p>부모가 설정을 완료하기 전에 자식이 렌더링됩니다. 구독이 준비되기 전에 자식에서 `useForm` 메서드(예: `setValue`)를 사용하면 문제가 발생할 수 있습니다. `isReady` 플래그를 사용하여 자식에서 상태를 업데이트하기 전에 폼이 초기화되었는지 확인하세요.</p><CodeArea withOutCopy rawData={`const {\n setValue,\n formState: { isReady }\n} = useForm();\n\n// Parent component: ✅ \nuseEffect(() => setValue('test', 'data'), []) \n\n// Children component: ✅ \nuseEffect(() => isReady && setValue('test', 'data'), [isReady])`}/></li></ul>                                                                                                                                                      |
