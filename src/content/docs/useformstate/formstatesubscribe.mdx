...
React Hook Form 컴포넌트로, `useFormState`와 동일한 기능을 제공합니다. 다른 컴포넌트 내부에서 훅을 사용하는 대신, JSX에서 `<FormStateSubscribe />`를 직접 사용하여 폼 상태에 구독하고 이를 렌더링할 수 있습니다.
...
| `render`   | <TypeText>Function</TypeText>            | 지정된 폼 필드의 폼 상태에 구독하고, 폼 상태가 변경될 때마다 자식 함수가 다시 렌더링됩니다. 이를 통해 상태를 수동으로 연결하지 않고도 JSX에서 선언적으로 폼 상태를 사용할 수 있습니다. |
