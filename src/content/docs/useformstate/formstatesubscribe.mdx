...
React Hook Form 컴포넌트로, `useFormState`와 동일한 기능을 제공하지만 컴포넌트 형태로 제공합니다. 다른 컴포넌트 내에서 훅을 사용하는 대신, JSX 내에서 `<FormStateSubscribe />`를 직접 사용하여 폼 상태에 구독하고 렌더링할 수 있습니다.
...
| `name`     | <TypeText>string \| string[] </TypeText> | 단일 인풋 이름, 해당 배열, 또는 모든 인풋의 formState 업데이트에 구독할 수 있습니다.                                                                                                                        |
