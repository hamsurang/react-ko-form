...
React Hook Form 컴포넌트로, `useFormState`와 동일한 기능을 제공하지만 컴포넌트 형태로 제공됩니다. 다른 컴포넌트 내부에서 훅을 사용하는 대신, JSX에서 `<FormStateSubscribe />`를 직접 사용하여 폼 상태에 구독하고 이를 렌더링할 수 있습니다.
...
| `name`     | <TypeText>string \| string[] </TypeText> | 단일 인풋 이름을 제공하거나, 이들의 배열을 제공하거나, 모든 인풋의 formState 업데이트에 구독할 수 있습니다.                                                                                                                        |
| `disabled` | <TypeText>boolean = false</TypeText>     | 구독을 비활성화하는 옵션입니다.                                                                                                                                                                                 |
| `exact`    | <TypeText>boolean = false</TypeText>     | 이 속성은 인풋 이름 구독을 위한 정확한 일치를 가능하게 합니다.                                                                                                                                                  |
