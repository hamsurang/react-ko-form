...
React Hook Form 컴포넌트로, `useFormState`와 동일한 기능을 제공합니다. 다른 컴포넌트 안에서 훅을 사용하는 대신, JSX에서 직접 `<FormStateSubscribe />`를 사용하여 폼 상태에 구독하고 렌더링할 수 있습니다.
...
| `name`     | <TypeText>string \| string[] </TypeText> | 단일 인풋 이름, 여러 인풋 이름의 배열을 제공하거나 모든 인풋의 폼 상태 업데이트에 구독합니다.                                                                                                                        |
