...
`useFieldArray`와 통합되어 배열 필드를 순회합니다. 이 메서드를 사용하라면 `useFieldArray`의 `fields` 속성을 전달해야 합니다.
...
렌즈와 함께 배열을 손쉽게 다루려면 `@hookform/lenses/rhf`에서 확장된 `useFieldArray`를 import하세요.
