...
이 커스텀 훅은 개별 폼 상태를 구독하고, 해당 커스텀 훅 수준에서 리렌더링을 격리할 수 있습니다. 이 훅은 폼 상태 구독과 관련하여 자체적인 범위를 가지므로, 다른 useFormState 나 useForm 에 영향을 주지 않습니다. 이 훅을 활용하면 규모가 크고 복잡한 폼 애플리케이션에서 리렌더링 영향을 줄일 수 있습니다.
