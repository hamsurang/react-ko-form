...
## 어떻게 접근 가능한 입력 에러 및 메시지를 만드나요? {#Howtocreateanaccessibleinputerrorandmessage}
...
## 클래스 컴포넌트에서 동작하나요? {#DoesitworkwithClassComponents}
...
## 어떻게 폼 값을 초기화하나요? {#Howtoinitializeformvalues}
...
- 반응형 `values`
  ...
  ```javascript copy
  function App() {
    const { data } = useQuery() // data는 { firstName: '', lastName: '' }를 반환
    const { register, handleSubmit } = useForm({
      values: data,
      resetOptions: {
        keepDirtyValues: true, // 변경된 필드는 그대로 유지하되, defaultValues는 업데이트
      },
    })
  }
  ```
...
## 첫 번째 키 입력이 동작하지 않는 이유는 무엇인가요? {#Whyisthefirstkeystrokenotworking}
